...
## 2026-07-17 — Spec for #113 (unified fertilizer picker on watering form)

Wrote `docs/specs/issue-113-fertilizer-picker.md` (no code changes). Key findings baked into the
spec: `SmartSuggestions.watering_recipe_scorer` + recipe scorers already exist but are unused
(pre-built for this feature — use for the no-pins fallback); `watering_recipe_controller.js` has
dead `unifiedBatch*`/`recipeIdHidden` targets referenced by no view (remove); `watering_params`
already permits all three ids so no write-path change; form renders only on waterings#new/#edit;
found a pre-existing gap — `Watering` never validates recipe/source/batch belong to the plant's
project (cross-project attach via crafted POST) — spec adds the validation. Plan: `pinned` boolean
on recipes + recipe_sources, chip-row picker partial modeled on `shared/_smart_select`, new
`fertilizer_picker` Stimulus controller, server-rendered search panel (no fetch; `for_recipe`/
`for_project` endpoints become removable), batch auto-resolution (most recent active; select
revealed only when ≥2 active), TDS autofill preserved via a dispatched event.
