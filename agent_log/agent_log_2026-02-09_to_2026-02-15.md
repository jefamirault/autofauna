...
**Plan:**
1. Create 4 migration files (recipe_sources, recipes, recipe_ingredients+recipe_batches, add recipe fields to plants+waterings)
2. Create 4 new models (RecipeSource, Recipe, RecipeIngredient, RecipeBatch)
3. Modify existing models (Plant, Watering, Project, Tank)
4. Add routes for recipe_sources, recipes, recipe_batches
5. Create 3 new controllers (RecipeSourcesController, RecipesController, RecipeBatchesController)
6. Create CRUD views for all 3 new resources
7. Modify watering form with recipe/batch/TDS fields + Stimulus controller
8. Modify plant form with recipe dropdown
9. Add Ransack recipe filter to plants index
10. Update plant card + show page with recipe display
11. Update sidebar navigation
12. Create 2 new Stimulus controllers (watering-recipe, nested-form)
...
### Migrations (4 files)
- `db/migrate/20260215100000_create_recipe_sources.rb` — recipe_sources table with project_id, name, tank_id, description
- `db/migrate/20260215100001_create_recipes.rb` — recipes table with project_id, name, description
- `db/migrate/20260215100002_create_recipe_ingredients_and_recipe_batches.rb` — recipe_ingredients join table + recipe_batches table
...
- `app/models/recipe_source.rb` — belongs_to project/tank, has_many recipe_ingredients/recipes
- `app/models/recipe.rb` — belongs_to project, has_many ingredients/sources/batches/plants/waterings, accepts_nested_attributes, ransackable
- `app/models/recipe_ingredient.rb` — belongs_to recipe/recipe_source
- `app/models/recipe_batch.rb` — belongs_to project/recipe, enum volume_units matching Watering, to_s/label methods
- Modified: Plant (recipe association + ransackable), Watering (recipe_batch/recipe + before_validation), Project (has_many recipe_sources/recipes/recipe_batches), Tank (has_many recipe_sources)

### Routes
- Added resources for recipe_sources, recipes, recipe_batches (with for_recipe collection route)

### Controllers (3 new, 2 modified)
- `app/controllers/recipe_sources_controller.rb` — Standard CRUD scoped to current_project
- `app/controllers/recipes_controller.rb` — CRUD with nested attributes for ingredients
- `app/controllers/recipe_batches_controller.rb` — CRUD + for_recipe JSON endpoint
- Modified: WateringsController (added tds/recipe_batch_id/recipe_id to params), PlantsController (added recipe_id to params, recipe_filters, carry forward recipe params in water action)

### Views (15 new, 6 modified)
- `app/views/recipe_sources/` — index, _form, new, edit, show
- `app/views/recipes/` — index, _form (with nested ingredient rows), new, edit, show
- `app/views/recipe_batches/` — index (active/inactive groups), _form, new, edit, show
- Modified: waterings/_form (recipe/batch/TDS fields with Stimulus), plants/_form (recipe dropdown), plants/index (Ransack recipe filter), plants/_plant_row (recipe display + carry forward params), plants/show (recipe display), plants/_timeline (TDS/recipe/batch on waterings)

### Stimulus Controllers (2 new)
- `app/javascript/controllers/watering_recipe_controller.js` — dynamic batch dropdown on recipe change, auto-fill TDS
- `app/javascript/controllers/nested_form_controller.js` — add/remove rows for nested attributes

### Navigation & Styling
- Updated sidebar: Recipes, Batches, Sources under Plants nav group
- Updated header_config for new controllers
- Added CSS for nested form rows, recipe filter, timeline tags

**User must run:** `cd /home/jef/autofauna && bin/rails db:migrate`
