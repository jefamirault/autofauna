...
export default class extends Controller {
  ...
  // Display mode switching (client-side)
  ...
  updateDisplayMode() {
    ...
    if (this.hasLocationGroupTarget) {
      this.locationGroupTargets.forEach(group => {
        group.style.display = this.displayModeValue === "location" ? "" : "none"
      })
    }

    if (this.hasRecipeGroupTarget) {
      this.recipeGroupTargets.forEach(group => {
        group.style.display = this.displayModeValue === "recipe" ? "" : "none"
      })
    }

    if (this.hasWateringGroupTarget) {
      this.wateringGroupTargets.forEach(group => {
        group.style.display = this.displayModeValue === "watering" ? "" : "none"
      })
    }
