...
| `location_diagram_controller` | Location show (#115): 2D drag-and-drop plant layout. Pointer-event drag between canvas and tray, normalized 0–1 centre coordinates, debounced batched `PATCH` to `locations#update_layout`, optional photo backdrop, arrow-key nudge, legend click-to-highlight. Only draggable in "Arrange" mode (`editableValue`, editors only) |
