...
- **Dual `location-filter` controller instances:** The plants index has two instances — one on `#headerSearch` (outer: owns `searchInput` and `clearSearchBtn` targets) and one inside `turbo-frame#plants-results` (inner: owns filter state, button targets, and count targets). Code that needs to cross instances must use `document.querySelector('[data-location-filter-target="..."]')` rather than `this.*Target`. When the frame reloads (Turbo navigation), only the inner controller reconnects; the outer controller persists.
