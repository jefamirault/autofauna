...
| `dynamic_header_controller` | Scroll-driven header collapse on plant/watering show/edit pages. Attached to `<main>`, intercepts wheel/touch events to smoothly interpolate header between expanded and collapsed states. Sets `--header-height`, `--graphic-opacity`, `--title-opacity` CSS custom properties continuously based on scroll delta. Content stays visually in place during collapse (no scrolling behind header). Only activates when content overflows; latches once started so collapse always completes fully. |
